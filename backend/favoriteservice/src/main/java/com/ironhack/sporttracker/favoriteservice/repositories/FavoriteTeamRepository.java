...
public interface FavoriteTeamRepository extends JpaRepository<FavoriteTeam, Long> {
    ...
    @Query(value = "SELECT team_id, Count(team_id) as count FROM favorite_team WHERE sport = 'FOOTBALL' GROUP BY team_id ORDER BY count(*) DESC LIMIT 5", nativeQuery = true)
